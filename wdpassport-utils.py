...
def unlock():
	...
	## Allow non-interactive use (scripts): read password from env if set.
	passwd = os.environ.get("WDPASSPORT_PASSWORD")
	if not passwd:
...
def main(argv): 
	...
	for disk_device in context.list_devices(subsystem='block', DEVTYPE='disk'):
		...
		# Skip optical devices (e.g. the bundled SanDisk Unlocker virtual CD),
		# which share the same USB parent as the real disk.
		if disk_device.get("ID_CDROM") == "1" or (disk_device.device_node or "").startswith("/dev/sr"):
			continue

		# Scan parent for device name. Stop at the first match so a single
		# disk isn't added twice (once for itself, once for its USB parent).
		device = disk_device
		while device is not None:
			if "ID_SERIAL" in device:
				if device.properties["ID_SERIAL"].startswith(("Western_Digital_My_", "SanDisk_Extreme")):
